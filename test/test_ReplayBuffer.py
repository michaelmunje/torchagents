...
class TestReplayBuffer(TestCase):
    def test_update(self):
        ...
        buffer.update(state=state2, action=action2, reward=reward2, next_state=next_state2)
        (s, a, r, ns) = buffer.get_random_batch(2)
        self.assertTrue(torch.eq(s[0], state1).all().numpy() or torch.eq(s[0], state2).all().numpy())
        self.assertFalse(torch.eq(s[0], action1).all().numpy())
        self.assertFalse(torch.eq(s[0], next_state1).all().numpy())
        self.assertTrue(torch.eq(a[0], action1).all().numpy() or torch.eq(a[0], action2).all().numpy())
        self.assertTrue(torch.eq(r[0], reward1).all().numpy() or torch.eq(r[0], reward2).all().numpy())
        self.assertTrue(torch.eq(ns[0], next_state1).all().numpy() or torch.eq(ns[0], next_state2).all().numpy())
