...
class OffPolicyFilteringAgent(Agent):
    def __init__(self, state_shape: Tuple,
        ...
        super(OffPolicyFilteringAgent, self).__init__(policy=policy,
        ...
        self._episodes = [Episode(buffer_size=1000, state_shape=self._state_shape)
    ...
    def filter_episodes(self) -> None:
        rewards = [e.reward.cpu().numpy()[0] for e in self._episodes[:self._current_episode_index]]
        ...
        self._episodes = [e for e in self._episodes[:self._current_episode_index]
                          if e.reward.cpu().numpy()[0] >= reward_bound]
    ...
    def reset_episodes(self):
        self._episodes = [Episode(buffer_size=10000, state_shape=self._state_shape)
                          for _ in range(self._max_episodes)]
        self._current_episode_index = 0
    ...
    def get_action(self, state: torch.Tensor) -> torch.Tensor:
        action_probs = self.get_action_distribution(state).cpu().numpy()[0]
        if self.off_policy:
            return torch.tensor(np.random.choice(len(action_probs), p=action_probs),
